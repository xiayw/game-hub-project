...
function getAngle(controlEl, point, angularOffset) {
  ...
  const angle = rectUtils.getPointAngle(rect, point);
...
function snapAngleToStep(value, step) {
  return utils.snapValueToStep(value, MIN_VALUE, MAX_VALUE, step);
}
    ...
    getControlProps() {
      return normalize.element({
        ...
        onPointerDown(event) {
          ...
          const composedPath = domQuery.getNativeEvent(event).composedPath();
