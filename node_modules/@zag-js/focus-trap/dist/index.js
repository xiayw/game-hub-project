...
var FocusTrap = class {
  constructor(elements, options) {
        ...
        const hasControlledChanges = mutations.some((mutation) => {
          ...
          if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
            return Array.from(mutation.addedNodes).some((node) => {
              ...
              const element = node;
              if (domQuery.hasControllerElements(element)) {
                return true;
              }
              if (element.id && !this.state.containers.some((c) => c.contains(element))) {
                return domQuery.isControlledByExpandedController(element);
              }
              return false;
            });
          }
    ...
    __publicField(this, "getInitialFocusNode", () => {
      ...
      if (node === void 0 || node && !domQuery.isFocusable(node)) {
        const activeElement = domQuery.getActiveElement(this.doc);
  ...
  updatePortalContainers() {
    ...
    this.state.containers.forEach((container) => {
      const controlledElements = domQuery.getControlledElements(container);
  ...
  isControlledElement(container, element) {
    ...
    return domQuery.isControlledElement(container, element);
  }
  updateTabbableNodes() {
    this.state.containerGroups = this.state.containers.map((container) => {
      const tabbableNodes = domQuery.getTabbables(container, { getShadowRoot: this.config.getShadowRoot });
      const focusableNodes = domQuery.getFocusables(container, { getShadowRoot: this.config.getShadowRoot });
      ...
      for (let i = 0; i < tabbableNodes.length; i++) {
        if (domQuery.getTabIndex(tabbableNodes[i]) > 0) {
      ...
      function nextTabbableNode(node, forward = true) {
        ...
        if (forward) {
          for (let i = focusableIdx + 1; i < focusableNodes.length; i++) {
            if (domQuery.isTabbable(focusableNodes[i])) return focusableNodes[i];
          }
        } else {
  ...
  activate(activateOptions) {
    ...
    this.state.paused = false;
    this.state.nodeFocusedBeforeActivation = domQuery.getActiveElement(this.doc);
    onActivate?.();
